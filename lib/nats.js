 ...
 * @api private
 */
Client.prototype.checkArgs = function (subject, callback, opts) {
  if (typeof callback !== 'function') {
    throw new NatsError('requests require a callback', ErrorCode.API_ERROR)
  }
  if (typeof subject !== 'string' || subject === '') {
    callback(new NatsError(BAD_SUBJECT_MSG, ErrorCode.BAD_SUBJECT))
    return false
  }
  if (this.closed) {
    callback(new NatsError(CONN_CLOSED_MSG, ErrorCode.CONN_CLOSED))
  ...
  if (typeof opts !== 'object') {
    callback(new NatsError(BAD_OPTS_MSG, ErrorCode.BAD_OPTIONS))
    return false
  }
  return true
}
...
Client.prototype.subscribe = function (subject, callback, opts) {
  opts = opts || {}
  if (!this.checkArgs(subject, callback, opts)) {
    return 0
  }
