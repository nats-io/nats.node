...
const BAD_OPTIONS_MSG = 'Options should be an object as second paramater.'
...
Client.prototype.parseOptions = function (opts) {
  ...
  if (Array.isArray(options.servers)) {
    options.servers.forEach((server) => {
      this.servers.push(new Server(new URL(server)))
    })
    // Randomize if needed
    ...
    if (options.url !== undefined && this.servers.indexOf(options.url) === -1) {
      ...
      this.servers.unshift(new Server(new URL(options.url)))
    }
  } else {
    if (undefined === options.url) {
      options.url = DEFAULT_URI
    }
    this.servers.push(new Server(new URL(options.url)))
  }
...
Client.prototype.addServer = function (uri) {
  this.servers.push(new Server(new URL(uri)))
