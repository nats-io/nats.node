...
var VERSION = '0.6.8',
    ...
    REQUEST_TIMEOUT = 'Request wait time elapsed without receiving a reply',
...
Client.prototype.request = function(subject, opt_msg, opt_options, callback) {
  ...
  // if a timeout is set on the options, capture it then remove it from
  // the options so it's not passed along
  if (opt_options !== null && opt_options !== undefined && typeof opt_options.timeout === "number") {
    timeout = opt_options.timeout;
    delete opt_options.timeout;
  }
 ...
 // if a timeout was set, wait that long, then throw an exception
