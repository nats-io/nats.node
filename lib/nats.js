...
var VERSION = '0.7.4',
    ...
    REQUEST_TIMEOUT = 'REQUEST_TIMED_OUT',
    REQUEST_TIMEOUT_MSG = 'Request timed ou, subscription ended.',
...
Client.prototype.requestOnce = function(subject, opt_msg, opt_options, callback) {
  ...
  if (!opt_options.max && opt_options.max !== false) {
    opt_options.max = 1;
  }
  this.request(subject, opt_msg, opt_options, callback);
});
...
Client.prototype.request = function(subject, opt_msg, opt_options, callback) {
  ...
  // To keep old behaviour, remove after major version bump
  function wrapCallback (error, msg, reply) {
    if (opt_options.timeout) {
      callback(error, msg, reply);
    } else {
      callback(msg, reply);
    }
  }
  var s = this.subscribe(inbox, opt_options, function(msg, reply) {
    wrapCallback(null, msg, reply);
  });
  this.publish(subject, opt_msg, inbox);
  
  if (opt_options.timeout) {
    var self = this;
    this.timeout(s, opt_options.timeout, opt_options.max, function() {
      self.unsubscribe(s);
      wrapCallback(new NatsError(REQUEST_TIMEOUT, REQUEST_TIMEOUT_MSG));
    });
  }
  return s;
};
