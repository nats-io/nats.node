...
Client.prototype.flushPending = function() {
  ...
  this.pending.forEach(function(cmd) {
      result = self.stream.write(cmd) && result;
  });
  this.pending = [];
  this.pSize   = 0;
  return result;
...
Client.prototype.sendCommand = function(cmd) {
  ...
  // this is a plain command but may contain utf-8 characters, such as SUB ready.√.steady.√ 1\r\n
...
Client.prototype.publish = function(subject, msg, opt_reply, opt_callback) {
  ...
  if (!Buffer.isBuffer(msg)) {
      // this assumes we want to publish strings in utf-8, even if different from this.encoding
      msg = new Buffer(msg);
  }
