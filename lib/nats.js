...
ErrorCode.CALLBACK_REQUIRED = 'CALLBACK_REQ'
...
const CALLBACK_REQUIRED_MSG = 'Callback is required and must be a function'
...
Client.prototype.processMsg = function () {
  ...
  if (sub !== undefined) {
    ...
    if (sub.callback) {
      ...
      try {
        const v = { data: msg, reply: this.payload.reply, subject: this.payload.subj, sid: this.payload.sid }
...
Client.prototype.handledClosedOrDraining = function (callback) {
  ...
  if (this.draining) {
    if (typeof callback === 'function') {
      callback(new NatsError(CONN_DRAINING_MSG, ErrorCode.CONN_DRAINING))
    } else {
      throw (new NatsError(CONN_DRAINING_MSG, ErrorCode.CONN_DRAINING))
    }
    return true
  }
}
 ...
 * @api public
 */
Client.prototype.publish = function (subject, msg, reply) {
  if (this.closed) {
    throw new NatsError(CONN_CLOSED_MSG, ErrorCode.CONN_CLOSED)
  }
  ...
  if (typeof subject !== 'string' || subject === '') {
    throw new NatsError(BAD_SUBJECT_MSG, ErrorCode.BAD_SUBJECT)
  }
  ...
  if (!Buffer.isBuffer(msg)) {
    ...
    if (this.options.json) {
      try {
        ...
        throw (new NatsError(BAD_JSON_MSG, ErrorCode.BAD_JSON))
      }
    }
...
Client.prototype.subscribe = function (subject, callback, opts) {
  ...
  if (typeof callback !== 'function') {
    throw new NatsError(CALLBACK_REQUIRED_MSG, ErrorCode.CALLBACK_REQUIRED)
  }
  opts = opts || {}
  if (typeof opts !== 'object') {
    throw new NatsError(BAD_OPTS_MSG, ErrorCode.BAD_OPTIONS)
  }
  ...
  if (opts.timeout) {
    sub.expected = opts.expected || 1
    sub.timeout = setTimeout(() => {
      this.unsubscribe(sub.ssid)
      sub.callback(new NatsError(TIMEOUT_MSG, ErrorCode.TIMEOUT_ERR))
    }, opts.timeout)
  }
  return this.ssid
}
      ...
      if (conf) {
        ...
        if (Object.hasOwnProperty.call(conf, 'expected')) {
          ...
          if (conf.received >= conf.expected) {
            ...
            this.emit('unsubscribe', sid, msg.subject)
          }
        }
      }
    })
