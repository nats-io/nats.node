...
function Client(opts) {
  ...
  var options = parseInitOptions(opts);
  var stream = net.createConnection(options.uri.port, options.uri.hostname);
  ...
  var cs = JSON.stringify({'verbose':false, 'pedantic':false});
  this.sendCommand('CONNECT ' + cs + CR_LF);
}
...
Client.prototype.request = function(subject, opt_msg, callback) {
  if (typeof opt_msg == 'function') {
    callback = opt_msg;
    opt_msg = EMPTY;
  }
  var inbox = createInbox();
  var s = this.subscribe(inbox, function(msg, reply) {
    callback(msg, reply);
  });
