...
Client.prototype.sendConnect = function() {
  ...
  // reconnected, there may have been other data pending, so this ensures we
  // will be authenticated and (re)subscribed and will have emitted a connect
  // or reconnect event before we flush pending requests.
  this.stream.write(CONNECT + SPC + JSON.stringify(cs) + CR_LF);
  this.sendSubscriptions();

  this.pongs.unshift(function() { client.connectCB(); });
...
Client.prototype.createConnection = function() {
  ...
  // sync our SUBs. Without this, users of the client may miss crucial state
  // transitions via callbacks, would have to track the client's internal
  // connection state, and may have to double buffer messages (which we are
  // already doing) if they wanted to ensure their messages reach the server.
  var pong = [];
  var pend = [];
  var pSize = 0;
  var client = this;
  if (client.pongs && client.pending) {
    var pongIndex = 0;
    client.pending.forEach(function(cmd) {
      var cmdLen = Buffer.isBuffer(cmd) ? cmd.length : Buffer.byteLength(cmd);
      if (cmd === PING_REQUEST && pongIndex < client.pongs.length) {
...
Client.prototype.flushPending = function() {
  ...
  var result = true;
  var buf = new Buffer(this.pSize);
  var pos = 0;
  this.pending.forEach(function(cmd) {
      var len;
...
Client.prototype.sendCommand = function(cmd) {
  ...
  this.pSize += Buffer.isBuffer(cmd) ? cmd.length : Buffer.byteLength(cmd);
...
Client.prototype.sendSubscriptions = function() {
  ...
  for (var sid in this.subs) {
	...
	proto = [SUB, sub.subject, sid + CR_LF];
      ...
      this.stream.write(proto.join(SPC));
    }
  }
};
...
Client.prototype.publish = function(subject, msg, opt_reply, opt_callback) {
  ...
  if (Buffer.isBuffer(msg)) {
    this.sendCommand(Buffer.concat([new Buffer(psub), new Buffer(msg.length.toString()), CR_LF_BUF, msg, CR_LF_BUF]));
  } else {
    this.sendCommand(psub + Buffer.byteLength(msg).toString() + CR_LF + msg + CR_LF);
  }
