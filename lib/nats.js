...
var VERSION = '0.5.0',
    ...
    // Errors
    PUB_BAD_SUBJECT_ERR = new Error('Publish: Subject must be supplied'),
    PUB_BAD_MSG_ERR = new Error('Publish: Message can\'t be a function'),
    PUB_BAD_REPLY_ERR = new Error('Publish: Reply can\'t be a function'),
    PUB_CONN_CLOSED_ERR = new Error('Publish: Connection closed'),
    FLUSH_CONN_CLOSED_ERR = new Error('Flush: Connection closed'),
    SUB_CONN_CLOSED_ERR = new Error('Subscribe: Connection closed'),
...
Client.prototype.flush = function(opt_callback) {
  if (this.closed) {
    if (typeof opt_callback === 'function') {
      opt_callback(FLUSH_CONN_CLOSED_ERR);
      return;
    } else {
      this.emit('error', FLUSH_CONN_CLOSED_ERR);
    }
  }
  if( this.pongs ) {
...
Client.prototype.publish = function(subject, msg, opt_reply, opt_callback) {
  ...
  if (typeof subject !== 'string') {
    this.emit('error', PUB_BAD_SUBJECT_ERR);
  }
  if (typeof msg === 'function') {
    if (opt_callback || opt_reply) {
      this.emit('error', PUB_BAD_MSG_ERR);
    }
    opt_callback = msg;
    msg = EMPTY;
    opt_reply = undefined;
  ...
  if (typeof opt_reply === 'function') {
    if (opt_callback) {
      this.emit('error', PUB_BAD_REPLY_ERR);
    }
  ...
  if (opt_callback !== undefined) {
    ...
    this.emit('error', PUB_CONN_CLOSED_ERR);
  }
};
...
Client.prototype.subscribe = function(subject, opts, callback) {
  if (this.closed) {
    this.emit('error', SUB_CONN_CLOSED_ERR);
  }
  var qgroup, max;
