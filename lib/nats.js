...
Client.prototype.processMsg = function () {
  ...
  if (sub !== undefined) {
    ...
    if (sub.callback) {
      ...
      try {
        sub.callback(undefined, { data: msg, replyTo: this.payload.reply, subject: this.payload.subj, sid: this.payload.sid })
      } catch (error) {
...
Client.prototype.subscribe = function (subject, callback, opts) {
  ...
  if (opts.timeout) {
    sub.expected = opts.expected
    sub.timeout = setTimeout(() => {
      this.unsubscribe(sub.ssid)
      sub.callback(new NatsError('timeout', 'timeout'))
    }, opts.timeout)
  }
  return this.ssid
}
