...
Client.prototype.maybeLog = function(args) {
    if(this.DEBUG) {
...
Client.prototype.setupHandlers = function() {
    ...
    stream.on('close', function(hadError) {
        client.maybeLog(Date.now(), " stream close");

        client.closeStream();
        client.emit('disconnect');
            client.maybeLog(Date.now(), " notified disconnected close", client.listeners('disconnect').length);
...
Client.prototype.processErr = function(s) {
    this.maybeLog(Date.now(), "PROCESS ERR");
    ...
    if (m.indexOf(STALE_CONNECTION_ERR) !== -1) {
        ...
        this.maybeLog(Date.now(), "PERMISSION ERROR FROM SOCKET");
        this.emit('permission_error', new NatsError(s, NATS_PROTOCOL_ERR));
    } else {
        this.maybeLog(Date.now(), " Error");
...
Client.prototype.reconnect = function() {
    ...
    this.maybeLog(Date.now(), "reconnect", this.reconnects);
