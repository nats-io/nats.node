...
Client.prototype.createResponseMux = function () {
  if (!this.respmux) {
    const inbox = this.createInbox()
    const ginbox = inbox + '.*'
    const sub = this.subscribe(ginbox, (err, msg) => {
      ...
      if (conf) {
        ...
        if (Object.hasOwnProperty.call(conf, 'expected')) {
          ...
          if (conf.received >= conf.expected) {
            ...
            this.emit(Events.Unsubscribe, { sid: conf.id, subject: msg.subject })
          }
        }
      }
    })
    ...
    this.respmux.subscriptionID = sub.sid
...
Req.prototype.cancel = function () {
  ...
  this.nc.cancelMuxRequest(this.sid)
}
