...
function Client(opts) {
  ...
  var stream = this.stream = net.createConnection(client.url.port, client.url.hostname);
  ...
  stream.on('end', function (exception) {
    if (client.options.reconnect === false ||
        client.reconnects > DEFAULT_MAX_RECONNECT_ATTEMPTS) {
      client.emit("disconnected", exception);
    } else {
