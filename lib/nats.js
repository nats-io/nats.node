 ...
 * @api private
 */
Client.prototype.checkSubArgs = function (subject, callback, opts) {
  if (typeof callback !== 'function') {
 ...
 * @api private
 */
Client.prototype.checkPubArgs = function (subject, reply, data, callback) {
  if (callback && typeof callback !== 'function') {
    this.dispatchError(callback, new NatsError('callback must be a function', ErrorCode.API_ERROR))
  }
...
Client.prototype.publishRequest = function (subject, reply, data, callback) {
  ...
  this.doPublish(subject, reply, data, callback)
}
...
Client.prototype.publish = function (subject, data, callback) {
  this.doPublish(subject, '', data, callback)
}
...
Client.prototype.doPublish = function (subject, reply, data, callback) {
  if (!this.checkPubArgs(subject, reply, data, callback)) {
    return
  }
  if (!this.options.json) {
    data = data || EMPTY
...
Client.prototype.subscribe = function (subject, callback, opts) {
  opts = opts || {}
  if (!this.checkSubArgs(subject, callback, opts)) {
    return 0
  }
...
Client.prototype.request = function (subject, callback, data, opts) {
  ...
  if (this.options.noMuxRequests) {
    ...
    this.publishRequest(subject, inbox, data)
    return sid
  } else {
    const conf = this.initMuxRequestDetails(callback, opts.max)
    this.publishRequest(subject, conf.inbox, data, callback)
    if (opts.timeout) {
