...
const BAD_OPTS = 'BAD_OPTS'
const BAD_REPLY = 'BAD_REPLY'
const BAD_SUBJECT = 'BAD_SUBJECT'
const CALLBACK_REQUIRED = 'CALLBACK_REQ'
...
Client.prototype.subscribe = function (subject, callback, opts) {
  ...
  if (typeof callback !== 'function') {
    throw new NatsError(CALLBACK_REQUIRED_MSG, CALLBACK_REQUIRED)
  }
  opts = opts || {}
  if (typeof opts !== 'object') {
    throw new NatsError(BAD_OPTS_MSG, BAD_OPTS)
  }
  ...
  if (opts.timeout) {
    ...
    sub.timeout = setTimeout(() => {
      ...
      sub.callback(new NatsError(TIMEOUT_MSG, TIMEOUT_ERR))
    }, opts.timeout)
  }
  return this.ssid
}
...
Client.prototype.request = function (subject, callback, data, opts) {
  ...
  if (opts.noMuxRequests) {
    opts.expected = opts.expected || opts.max
    const inbox = nuid.next()
